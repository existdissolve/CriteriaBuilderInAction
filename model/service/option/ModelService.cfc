component extends="CarTracker.model.service.BaseService" {
     ...
     * Constructor
     */
    public ModelService function init() {
    	super.init( entityName="Model" );
    	// use query caching
     ...
     * returns CarTracker.model.orm.option.Model
     */
    public CarTracker.model.orm.option.Model function populate(
   		required Any target="",
    	...
    	arguments.exclude = listAppend( arguments.exclude, "ModelID" );
    ...
    private CriteriaBuilder function buildCriteria( required Any criteria={} ) {
    	...
    	for( var crit in arguments.criteria ) {
            ...
            if( crit=="ModelID" ) {
                c.isEq( "ModelID", arguments.criteria[ crit ] );
            }
    	}
    	return c;
    }
